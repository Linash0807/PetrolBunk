...
const navItems = [
  ...
  { name: 'Shift Entry', icon: PlusCircle, path: '/new-entry' },
  { name: '24hrs Report', icon: PlusCircle, path: '/24hrs' },
...
export function Sidebar({ isOpen, onClose, activePath = '/', onNavigate }: SidebarProps) {
  return (
    <>
      ...
      {/* Sidebar sidebar */}
      <aside 
        className={`fixed inset-y-0 left-0 z-50 w-64 glass-effect transform transition-transform duration-300 ease-in-out md:translate-x-0 md:static md:inset-auto md:w-64 max-md:bg-white max-md:dark:bg-slate-900 print:hidden ${
